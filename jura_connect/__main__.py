...
def cmd_command(args: argparse.Namespace) -> int:
    ...
    # --pin overrides the stored PIN; an unset flag falls back to the
    # credential store so paired PIN machines reconnect without re-typing.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pa.add_argument(
        "--pin",
        help=(
            "front-panel setup PIN to include in the @HP handshake (e.g. Jura "
            "E6 / EF1030); stored with the credential so reconnects reuse it"
        ),
    )
    pa.add_argument(
        "--timeout",
    ...
    cm.add_argument(
        "--pin",
        help=(
            "front-panel setup PIN for the @HP handshake; overrides the "
            "stored PIN (default: use the stored PIN, if any)"
        ),
    )
    cm.add_argument(
        "--machine-type",
    ...
    cm.add_argument(
        ...
        help=(
            ...
            "(clean / descale / set-pin / set-ssid / skip-quality-step / …). "
