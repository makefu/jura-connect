...
Note that `JuraClient.close()` still sends a bare `@HE` as a "polite
close" — that predates the discovery that `@HE` is the OTA end marker
(tracked in `JOE_GAPS.md` §8.3). Outside an OTA session the dongle
answers `@he:error`, which is why it has looked harmless.
