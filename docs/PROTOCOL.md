...
own room database. `@TG:FF` is already in `DESTRUCTIVE_PREFIXES` and
reachable as `raw '@TG:FF'`; whether it actually clears a *pending*
timer (as opposed to aborting a running brew) is untested.
